...
public class PlayerInput : MonoBehaviour
{
    ...
    private void ProcessPlayerInputNew()
    {
        ...
        var grabButtonControl =
            (ButtonControl) controls.Player.Grab.controls[0];

        // Check if the grab button was pressed this frame (comparable to GetButtonDown)
        bool grabPressedThisFrame =
            (
            grabButtonControl != null && grabButtonControl.wasPressedThisFrame
            );
        ...
        bool grabHeldThisFrame =
            (grabButtonControl != null && grabButtonControl.isPressed);

        // Or was pressed in a previous frame, but was not yet cleared
        grabHeld = grabHeld || grabHeldThisFrame;

        // Check if the grab button was released this frame (comparable to GetButtonUp)
        bool grabReleasedThisFrame =
            (
            grabButtonControl != null && grabButtonControl.wasReleasedThisFrame
            );
